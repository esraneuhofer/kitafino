...
async function sendEmailDailyConfirmationWeek(weekplan, settings, customer, currentWeek, currentYear, monday, studentsCustomer) {
    ...
    try {
        ...
        // Notify admin
        await notifyAdmin(error, customer.contact.email,'Failed to send daily confirmation email');
...
async function notifyAdmin(error, customerEmail, customerId, consoleMessage) {
    const msg = {
        ...
        html: `<p>An Kunde: ${customerEmail}</p>
<p>An error occurred: Message: ${error.message}</p><br>
<p>Stack: ${error.stack}</p>
<pre>Console: ${consoleMessage}</pre>
...
module.exports = {sendEmailDailyConfirmationWeek, notifyAdmin};
