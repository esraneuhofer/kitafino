...
async function processOrderWeekly(customerId, tenantId) {
    ...
    try {
            ...
            for (let indexDay = 0; indexDay <= 5; indexDay++) {
                ...
                if (isHoliday(day, customer.settings.state) || isVacation(day, vacationCustomer)) {
                    try {
                        ...
                        console.error('Failed to send cancellation email Holiday Or Vacation:', error.message);
                        await notifyAdmin(error, customer$.contact.email, customerId, 'Failed to send cancellation email Holiday Or Vacation, line 63 task-weekly-order-deadline.js');
                    }
                } else {
                    ...
                    try {
                        ...
                        try {
                            ...
                            console.error('Failed to send success email line 78 task-weekly-order-deadline.js', error.message);
                            await notifyAdmin(error, customer$.contact.email, customerId, 'Failed to send success email line 78 task-weekly-order-deadline.js')
                        }
                    } catch (error) {
                        console.error('Failed to add order line 82 task-weekly-order-deadline.js', error.message);

                        if(error.message !== 'Ein doppelter Eintrag wurde erkannt. Bitte überprüfen Sie Ihre Bestellung.'){
                            await notifyAdmin(error, customer$.contact.email, customerId, 'Failed to add order line 82 task-weekly-order-deadline.js')
                        }
                        try {
                            await sendCancellationEmail(mockReq, error.message);
                        } catch (emailError) {
                            console.error('Failed to send cancellation email: line 94 task-weekly-order-deadline.js:', emailError.message);
                            await notifyAdmin(emailError, customer$.contact.email, customerId, 'Failed to send cancellation email: line 94 task-weekly-order-deadline.js')
                        }
                    }
                }
            }
        ...
        try {
            ...
            await notifyAdmin(error, customer$.contact.email, customerId, 'Failed to send daily confirmation Email Customer and Tenatn:: line 105 task-weekly-order-deadline.js')


        }
    } catch (error) {
        await notifyAdmin(error, customer$.contact.email, customerId, 'Failed to check deadlines and send order email: line 110 task-weekly-order-deadline.js')
...
const testing = async (req, res) => {
    try {
        ...
        let customerIdString = "667ed0fd693c21000281fdfb";
