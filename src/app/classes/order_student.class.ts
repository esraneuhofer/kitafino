...
class OrderModelSingleDayStudent implements OrderInterfaceStudentDay {
  ...
  constructor(customer: CustomerInterface,
    ...
    if (studentModel && studentModel.specialFood && !customer.generalSettings.showAllSpecialFood && !settings.orderSettings.specialShowPerMeal) {
      ...
      let specialFood = getSpecialFoodById(studentModel.specialFood, settings);
      if (!specialFood) return;
      const orderSpecial: OrderSubDetailNew = setOrderSpecialFood(priceSpecial, specialFood, '', null)
      this.orderMenus.push(orderSpecial);
    }
...
function setOrderSpecialFood(priceStudent: number, specialFood: SpecialFoodInterface, nameMenu = '', idMenuMain: null | string): OrderSubDetailNew {

  console.log('nameMenu Food', nameMenu)
  let order: OrderSubDetailNew = {
    menuSelected: false,
    priceOrder: priceStudent,
    nameMenu: nameMenu + specialFood.nameSpecialFood,
    displayMenu: true,
    isDge: false,
    typeOrder: 'specialFood',
    nameOrder: nameMenu + specialFood.nameSpecialFood,
