...
export function getEmailBodyCancel(objectData: EmailOrderInterface,price:number): any {
  ...
  const textEmail = getTextCancel(objectData,price);
  const emailBody = getEmailBodyHtml(objectData);
...
function getEmailBodyHtml(objectData: EmailOrderInterface): string {
  let string = '';
